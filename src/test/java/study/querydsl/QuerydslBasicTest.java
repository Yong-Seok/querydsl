...
//        QMember m = QMember.member;
     ...
     * SPQL: select m, t from Member m left join m.team t on t.name = 'teamA'
     */
    @Test
...
//                .join(member.team, team).on(team.name.eq("teamA"))    // team null은 제외
                .fetch();
    ...
    public void subQueryEq() throws Exception {
        // com.querydsl.jpa.JPAExpressions 이용한 서브쿼리 작성. static import
    ...
    public void findDtoByJPQL() throws Exception {
        // 순수 JPA에서 DTO 조회 시 package 포함된 생성자 사용해야 함
    ...
    public void findDtoByQueryProjection() throws Exception {
        List<MemberDto> result = queryFactory
                .select(new QMemberDto(
                        member.username,
                        member.age
                        // Compile Exception -> Constructor 차이점
                        // , member.id
                ))
                .from(member)
                .fetch();
    ...
    private List<Member> searchMember1(String usernameParam, Integer ageParam) {
        BooleanBuilder builder = new BooleanBuilder(
                // 초기값 세팅 가능
                member.username.eq(usernameParam)
        );
...
//                        usernameEq(usernameParam), ageEq(ageParam)
    ...
    public void bulkUpdate() throws Exception {
        ...
        // 벌크연산은 DB 데이터와 영속성 컨텍스트의 데이터가 다름
        // 영속성 컨텍스트의 데이터가 DB 데이터보다 우선됨
        // 벌크연산 후 영속성 컨텍스트를 강제로 DB에 밀어넣고(flush) 비워준다(clear)
        em.flush();
        em.clear();
